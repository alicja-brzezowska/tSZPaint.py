...
def plot_scatter_comparison(python_map, julia_map, save_path=None):
    ...
    ax.scatter(
        jl_vals[both_positive], py_vals[both_positive], alpha=0.3, s=1, c="steelblue"
    )
    ax.set_xscale("log")
    ax.set_yscale("log")
    lims_log = [
        min(py_vals[both_positive].min(), jl_vals[both_positive].min()) * 0.5,
...
def plot_histogram_comparison(python_map, julia_map, save_path=None):
    ...
    ax.hist(
        ...
        bins=bins,
        alpha=0.6,
        label=f"Python (n={len(py_nonzero)})",
        color="steelblue",
    )
    ax.hist(
        jl_nonzero,
        bins=bins,
        alpha=0.6,
        label=f"Julia (n={len(jl_nonzero)})",
        color="coral",
    )
...
def print_statistics(python_map, julia_map):
    ...
    print(
        f"{'Mean (non-zero)':<25} {python_map[mask].mean():<15.6e} {julia_map[mask].mean():<15.6e}"
    )
    print(
        f"{'Non-zero pixels':<25} {(python_map > 0).sum():<15} {(julia_map > 0).sum():<15}"
    )
    ...
    print(
        f"{'  Correlation':<25} {np.corrcoef(python_map[mask], julia_map[mask])[0, 1]:<15.6f}"
    )
...
def main():
    ...
    if python_weighted is not None:
        ...
        print(
            f"  Ratio (weighted/no_wt):     {python_weighted.sum() / python_map.sum():.4f}"
        )

    # Histogram
    plot_histogram_comparison(
        python_map, julia_map, save_path=OUTPUT_DIR / "comparison_histogram.png"
    )

    # Scatter plot
    plot_scatter_comparison(
        python_map, julia_map, save_path=OUTPUT_DIR / "comparison_scatter.png"
    )
