...
import asdf as asdf_lib
from abacusnbody.data.bitpacked import unpack_rvint
from abacusnbody.data.compaso_halo_catalog import CompaSOHaloCatalog
from abacusnbody.data.read_abacus import read_asdf

from abacusnbody.analysis.shear import get_shear, smooth_density
from abacusnbody.analysis.tsc import tsc_parallel
from abacusnbody.hod.menv import do_Menv_from_tree

DEFAULTS = {}
DEFAULTS['path2config'] = 'config/abacus_hod.yaml'


class HaloTable(dict):
    """Dict wrapper so len() returns number of halos, not number of keys."""
    def __len__(self):
        return len(self['N'])

    def __getitem__(self, key):
        if isinstance(key, (np.ndarray, slice)):
            return HaloTable({k: v[key] for k, v in self.items()})
...
def gen_rand(N, chi_min, chi_max, fac, Lbox, offset, origins, rng, lc_box_repeats=1):
    ...
    if origins.shape[0] > 1:
        # Standard 3-box L-shaped lightcone (original logic unchanged)
        ...
        vert0 = box0 + vert
        if chi_max >= (Lbox - offset):
            vert1 = box1 + vert
            vert2 = box2 + vert
            mask = is_in_cube(x_cart, y_cart, z_cart, vert0) \
                 | is_in_cube(x_cart, y_cart, z_cart, vert1) \
                 | is_in_cube(x_cart, y_cart, z_cart, vert2)
        else:
            mask = is_in_cube(x_cart, y_cart, z_cart, vert0)
    else:
        # Full-sky lightcone (single origin), possibly with tiled box repeats.
        # A random point is valid if it falls inside ANY tiled copy of the box.
        # Box tiles are centred at (i*Lbox, j*Lbox, k*Lbox) for i,j,k in [-nr, nr].
        half = Lbox / 2.0 - offset  # effective half-side after edge removal
        nr = int(lc_box_repeats)
        mask = np.zeros(N_rands, dtype=bool)
        for i in range(-nr, nr + 1):
            for j in range(-nr, nr + 1):
                for k in range(-nr, nr + 1):
                    ...
                    cz = k * Lbox - origin[2]
                    mask |= (
                        (np.abs(x_cart - cx) < half)
                        & (np.abs(y_cart - cy) < half)
                        & (np.abs(z_cart - cz) < half)
                    )
...
def prepare_slab(
    ...
    if halo_lc:
        # --- backlight lightcone format (lightcone_halo_info_000.asdf) ---
        halo_slabname = (
            simdir
            + '/'
            + simname
        ...
        part_slabname = (
            ...
            + '/z'
            + str(z_mock).ljust(5, '0')
            + '/lightcone_rvpid_000.asdf'
        )
        with asdf_lib.open(halo_slabname, lazy_load=False) as af:
            header = dict(af['header'])
            lc = af['halo_lightcone']
            ts = af['halo_timeslice']
            idx = np.asarray(lc['halo_timeslice_index'])

            INT16SCALE = 32000.0
            Lbox = float(header['BoxSizeHMpc'])
            r100 = np.asarray(ts['r100_L2com'])[idx]

            halos = HaloTable({
                'x_L2com':      np.asarray(lc['Interpolated_x_L2com']),
                'v_L2com':      np.asarray(lc['Interpolated_v_L2com']),
                'N':            np.asarray(lc['Interpolated_N']),
                'id':           np.asarray(ts['haloindex'])[idx],
                'npstartA':     np.asarray(ts['npstartA'])[idx],
                'npoutA':       np.asarray(ts['npoutA'])[idx],
                'sigmav3d_L2com': np.asarray(ts['sigmav3d_L2com'])[idx],
                'r90_L2com':    np.asarray(ts['r90_L2com_i16'])[idx] * r100 * Lbox / INT16SCALE,
                'r25_L2com':    np.asarray(ts['r25_L2com_i16'])[idx] * r100 * Lbox / INT16SCALE,
                'r98_L2com':    np.asarray(ts['r98_L2com_i16'])[idx] * r100 * Lbox / INT16SCALE,
            })

        if cleaning:
            mask = halos['N'] > 0
            halos = HaloTable({k: v[mask] for k, v in halos.items()})

        # load and unpack particles
        with asdf_lib.open(part_slabname, lazy_load=False) as pf:
            rvint = np.asarray(pf['data']['rvint'])
        pos_p, vel_p = unpack_rvint(rvint, Lbox)
        # build structured array matching the fields prepare_slab writes
        parts_dt = np.dtype([
            ('pos',            np.float32, 3),
            ('vel',            np.float32, 3),
            ('ranks',          np.float64),
            ...
            ('ranksp',         np.float64),
            ('ranksc',         np.float64),
            ('downsample_halo', np.float64),
            ('halo_vel',       np.float32, 3),
            ('halo_mass',      np.float64),
            ('Np',             np.float64),
            ('halo_id',        np.int64),
            ('randoms',        np.float64),
            ('halo_deltac',    np.float64),
            ('halo_fenv',      np.float64),
            ('halo_shear',     np.float64),
        ])
        parts = np.empty(len(pos_p), dtype=parts_dt)
        parts['pos'] = pos_p
        parts['vel'] = vel_p
        del rvint, pos_p, vel_p
        ...
        if z_type == 'primary':
            cat = CompaSOHaloCatalog(
                ...
                fields=[N_key, pos_key, vel_key, 'r90_L2com', 'r25_L2com', 'r98_L2com',
                        'npstartA', 'npoutA', id_key, 'sigmav3d_L2com'],
                cleaned=cleaning,
            )
        else:
            cat = CompaSOHaloCatalog(
        ...
        header = cat.header
        Lbox = float(cat.header['BoxSizeHMpc'])
    ...
    print(f'  halo+particle loading took {time.time() - t0:.1f}s')
    t0 = time.time()
    ...
    if want_AB:
        ...
        if halo_lc:
            ...
            origins = np.array(header.get('LightConeOrigins', header.get('LCOrigins', [[0,0,0]]))).reshape(-1, 3)
            # Force observer to [0,0,0] for lightcone geometry
            if not np.allclose(origins[0], [0,0,0]):
                print(f"WARNING: LightConeOrigins was {origins[0]}, forcing to [0,0,0] for correct lightcone geometry.")
                origins[0] = [0,0,0]
            lc_box_repeats = int(header.get('LCBoxRepeats', 1))
            ...
            if origins.shape[0] == 1:
                # Full-sky tiled lightcone: outer boundary is at ±lc_box_repeats * Lbox/2
                tiled_half = lc_box_repeats * Lbox / 2.0 - offset - rad_outer
                x_min_edge = -tiled_half
                x_max_edge =  tiled_half
                y_min_edge = -tiled_half
                y_max_edge =  tiled_half
                z_min_edge = -tiled_half
                z_max_edge =  tiled_half
            else:
                # Standard 3-box L-shaped lightcone (original logic)
            ...
            if len(index_bounds) > 0:
                ...
                if origins.shape[0] == 1:
                    tiled_half2 = lc_box_repeats * Lbox / 2.0 - offset - 2.0 * rad_outer
                    x_min_edge = -tiled_half2
                    x_max_edge =  tiled_half2
                    y_min_edge = -tiled_half2
                    y_max_edge =  tiled_half2
                    z_min_edge = -tiled_half2
                    z_max_edge =  tiled_half2
                else:
                ...
                # this is the number density (randoms generated uniformly on full sphere then
                # masked to tiled box volume; denominator is the full-sphere shell volume)
                if origins.shape[0] == 1:
        ...
        print(f'  assembly bias edge correction took {time.time() - t0:.1f}s')
        t0 = time.time()
        print('computing Menv (KD tree, this is the slow part)...')
        ...
        print(f'  Menv/fenv_rank took {time.time() - t0:.1f}s')
        t0 = time.time()
    ...
    if z_type == 'primary' or z_type == 'lightcone':
        ...
        print(f'  c rank took {time.time() - t0:.1f}s')
        t0 = time.time()
        print('compiling particle subsamples')
        start_tracker = 0
        for j in range(len(halos)):
            if j % 1000000 == 0:
                print(f'  halo {j}/{len(halos)} ({100*j/len(halos):.0f}%) elapsed {time.time()-t0:.0f}s', flush=True)
    ...
    print(f'  particle compilation loop took {time.time() - t0:.1f}s')
    t0 = time.time()
    ...
    masked = halos[mask_halos]
    dtype_list = []
    for k, v in masked.items():
        arr = np.asarray(v)
        if arr.ndim == 1:
            dtype_list.append((k, arr.dtype))
        else:
            dtype_list.append((k, arr.dtype, arr.shape[1:]))
    struct = np.empty(len(masked), dtype=dtype_list)
    for k in masked:
        struct[k] = masked[k]
    newfile.create_dataset('halos', data=struct)
    newfile.close()
...
def main(
    ...
    if halo_lc:
        halo_info_fns = [
            str(
                Path(simdir) / Path(simname) / ('z%4.3f' % z_mock) / 'lightcone_halo_info_000.asdf'
            )
        ]
    else:
        search_path = (
