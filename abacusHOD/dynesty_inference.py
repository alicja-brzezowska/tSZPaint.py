...
from HOD_lhc_emulator import load_emulator, predict
...
# From Liu 2025
_corr    = np.load( "/home/ab2927/rds/tSZPaint.py/ACT_forward_model/fig6_pz2.npy")
COV_DATA = np.diag(ACT_ERR) @ _corr @ np.diag(ACT_ERR)

EMULATOR_PKL = Path("/home/ab2927/rds/hpc-work/tSZPaint_data/stacked_profiles/hod_lhc_stacked.pkl")
OUT_DIR      = Path("/home/ab2927/rds/hpc-work/tSZPaint_data/inference")
OUT_DIR.mkdir(exist_ok=True)
OUT_PKL      = OUT_DIR / "dynesty_hod.pkl"
OUT_CORNER   = OUT_DIR / "corner_hod.png"


_npz        = np.load(EMULATOR_PKL.with_suffix(".npz"), allow_pickle=True)
PARAM_NAMES = list(_npz["param_names"])
PRIOR_BOUNDS = [
    (float(_npz["params"][:, i].min()), float(_npz["params"][:, i].max()))
